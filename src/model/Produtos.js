...
const getUsers = (request, response) => {
  database.pool.query('SELECT * FROM mercearia.produtos ORDER BY DESCRICAO ASC', (error, results) => {
    if (error) {
...
const getUserById = (request, response) => {
  ...
  database.pool.query('SELECT * FROM mercearia.produtos WHERE id = $1', [id], (error, results) => {
    if (error) {
